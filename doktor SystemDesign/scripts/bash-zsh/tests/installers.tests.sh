...
# "doktor" (install-doktor-bash-zsh.sh).
#
...
#   1. Instalar cria o bloco com a funcao "doktor" no .bashrc.
...
#   6. End-to-end: "doktor" baixa os arquivos de um repo local (file://),
#      e idempotente e registra a pasta no .gitignore do repositorio de
#      trabalho (pulado se faltar git ou rsync).
#
# Uso:
...
INSTALLER="$HERE/../install-doktor-bash-zsh.sh"
...
run_installer() {
  HOME="$TEST_HOME" SHELL=/bin/bash DOKTOR_REPO_URL="${DOKTOR_REPO_URL:-}" \
...
test_case '1. Instalar cria o bloco da funcao "doktor" no .bashrc'
run_installer; rc=$?
...
grep -q 'doktor()' "$TEST_HOME/.bashrc"; assert 'funcao doktor definida no bloco' $?
end_case
...
# =============================================================================
#  End-to-end: comando "doktor" contra um repositorio local (sem rede)
# =============================================================================

if command -v git >/dev/null 2>&1 && command -v rsync >/dev/null 2>&1; then
  test_case '6. doktor baixa de repo local, e idempotente e ajusta o .gitignore'

  # Repositorio "fonte da verdade" local.
  src="$TEST_HOME/fonte"
  mkdir -p "$src/core"
  printf '# doc de teste\n' > "$src/core/DOC.md"
  ...
  # Instala apontando para a fonte local e roda "doktor" num repo de trabalho.
  DOKTOR_REPO_URL="file://$src" run_installer
  work="$TEST_HOME/projeto"
  mkdir -p "$work"
  git -C "$work" init --quiet

  run_doktor() {
    HOME="$TEST_HOME" bash -c 'source "$HOME/.bashrc" && cd "$1" && doktor' _ "$work" </dev/null 2>&1
  }

  out1="$(run_doktor)"; rc=$?
  assert 'doktor saiu com codigo 0' "$rc" "codigo=$rc"
  dest="$work/doktor SystemDesign"
  [ -f "$dest/core/DOC.md" ]; assert 'arquivos baixados no destino' $?
  grep -qxF 'doktor SystemDesign/' "$work/.gitignore"; \
    assert 'pasta registrada no .gitignore do projeto' $?

  out2="$(run_doktor)"
  printf '%s' "$out2" | grep -q 'Nenhuma mudanca\|Ja estava atualizado'; \
    assert 'segunda execucao nao muda nada (idempotente)' $? "saida: $(printf '%s' "$out2" | tail -n 2)"
  n="$(grep -cxF 'doktor SystemDesign/' "$work/.gitignore")"
  [ "$n" -eq 1 ]; assert 'entrada no .gitignore aparece 1x' $? "apareceu $n vez(es)"
  end_case
else
  skip_case '6. End-to-end do comando doktor' 'git e/ou rsync nao estao no PATH.'
fi
